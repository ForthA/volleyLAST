...
class Data {
    ...
    public long insertGame(GameInfo gameInfo) {
        ...
        contentValues.put(TABLE_ROW_TSM, gameInfo.timeMillis);
    ...
    public ArrayList<GameInfo> writeGames(){
        ...
        if (cursor.moveToFirst()) {
            ...
            do {
                int id = Integer.parseInt(cursor.getString(idIndex));
                long tsm = Long.parseLong(cursor.getString(tsmIndex));
                gameInfos.add(new GameInfo(id, cursor.getString(dateIndex), cursor.getString(team1Index), cursor.getString(scoreIndex), cursor.getString(team2Index), tsm));
            } while (cursor.moveToNext());
        } else {
            Log.d("mLog", "0 rows");
        }
